	...
	try {
		...
		for (const child of children) {
			if (child.background) {
				...
				gherkinDocument.feature.children = children.filter((scenario) => scenario !== child)
			}
			break
		}
		...
		const generatedFeaturesDir = filepath.join(__dirname, '../e2e/features/generated_features')
		...
		const filePath = filepath.join(generatedFeaturesDir, fileName.replace('.feature', '.generated.feature'))
